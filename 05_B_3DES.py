...
def str_to_bitlist(s):
    return [(ord(ch) >> i) & 1 for ch in s for i in reversed(range(8))]

def bitlist_to_str(b):
    return ''.join(chr(int(''.join(str(x) for x in b[i:i+8]), 2)) for i in range(0, len(b), 8))
...
def des_block(block, keys, encrypt=True):
    ...
    for i in range(16):
        subkey = keys[i] if encrypt else keys[15-i]
        temp = feistel(R, subkey)
        L, R = R, xor(L, temp)
    block = R + L
    return permute(block, FP)

# DES encrypt/decrypt
def des(data, key, encrypt=True):
    keys = generate_keys(key)
    bit_data = str_to_bitlist(data)
    out = []
    for i in range(0, len(bit_data), 64):
        block = bit_data[i:i+64]
        out.extend(des_block(block, keys, encrypt))
    return bitlist_to_str(out)

# 3DES (EDE mode)
def triple_des_encrypt(data, key1, key2, key3):
    c1 = des(data, key1, True)
    c2 = des(c1, key2, False)
    c3 = des(c2, key3, True)
    return c3

def triple_des_decrypt(data, key1, key2, key3):
    c1 = des(data, key3, False)
    c2 = des(c1, key2, True)
    c3 = des(c2, key1, False)
    return c3
